<?php

namespace Tests\Feature;

use App\SalesForceClient;
use App\Search;
use App\User;
use Carbon\Carbon;
use Illuminate\Support\Facades\App;
use Illuminate\Support\Facades\Auth;
use Tests\TestCase;
use Illuminate\Foundation\Testing\RefreshDatabase;
use GuzzleHttp\Client;
use GuzzleHttp\Handler\MockHandler;
use GuzzleHttp\HandlerStack;
use GuzzleHttp\Psr7\Response;

class SearchControllerTest extends TestCase
{
    use RefreshDatabase;

    /**
     * @covers \App\Http\Controllers\SearchController::__construct
     * @covers \App\Http\Controllers\SearchController::historyIndex
     */
    public function testHistoryIndex()
    {
        $user = factory(\App\User::class)->create();

        $this->be($user);

        $mockClient = \Mockery::mock(SalesForceClient::class);
        App::instance(SalesForceClient::class, $mockClient);

        $shouldReturn = [];

        $this->get('api/search/history/all')
            ->assertStatus(200)
            ->assertJson($shouldReturn);
    }

    /**
     * @covers \App\Http\Controllers\SearchController::__construct
     * @covers \App\Http\Controllers\SearchController::show
     */
    public function testShow()
    {
        $record = factory(Search::class)->create();
        $record->save();

        $user = factory(\App\User::class)->create();
        $user->guid = "23705966";

        $this->be($user);

        $mock = new MockHandler([
            new Response(200, [], "{\"access_token\":\"00D5C000000LOtI!ARMAQO0V8C30qnwN2J3GPz9IOkdnQcP1ZSXp3abyzpRYJ92ORwLodp5s7D783BIy1IFrzOWSZwo_fRaY538nMc2SP7oO29u_\",\"instance_url\":\"https://pfizerbt--Haigang.cs62.my.salesforce.com\",\"id\":\"https://test.salesforce.com/id/00D5C000000LOtIUAW/0055C000000ooQHQAY\",\"token_type\":\"Bearer\",\"issued_at\":\"1543873716850\",\"signature\":\"A+i8lz+UTUTDwF9lk28LIB11oyPJAcPMVkdl5ANbL7E=\"}"),
            new Response(200, [], "{\"totalProject\":0,\"errorCode\":\"200\"}"),
        ]);

        $handler = HandlerStack::create($mock);
        $mock = new Client(['handler' => $handler]);
        $this->app->instance(Client::class, $mock);


        $this->get('api/search/history/1')
            ->assertStatus(200)
            ->assertJsonFragment([
                'totalProject' => 0
            ]);
    }

    /**
     * @covers \App\Http\Controllers\SearchController::__construct
     * @covers \App\Http\Controllers\SearchController::savedStore
     */
    public function testSavedStore()
    {
        $user = factory(User::class)->create();

        $this->be($user);

        $shouldReturn = [
            "filters" => [],
            "basic_search_text" => "foo"
        ];

        $this->post('api/search/saved', ['basicSearchText' => 'foo', 'is_saved' => true, 'useCached' => true])
            ->assertStatus(201)
            ->assertJsonFragment($shouldReturn);
    }

    /**
     * @covers \App\Http\Controllers\SearchController::__construct
     * @covers \App\Http\Controllers\SearchController::savedIndex
     */
    public function testSavedIndex()
    {
        $user = factory(\App\User::class)->create();

        $this->be($user);

        $mockClient = \Mockery::mock(SalesForceClient::class);
        App::instance(SalesForceClient::class, $mockClient);

        $this->get('api/search/saved/all')
            ->assertStatus(200)
            ->assertJson([]);
    }

    /**
     * @covers \App\Http\Controllers\SearchController::__construct
     * @covers \App\Http\Controllers\SearchController::savedDestroy
     */
    public function testSavedDestroy()
    {
        $user = factory(\App\User::class)->create();
        $user->guid = "23705966";

        $this->be($user);

        $record = factory(Search::class)->create();
        $record->save();

        $mockClient = \Mockery::mock(SalesForceClient::class);
        App::instance(SalesForceClient::class, $mockClient);

        $this->post('api/search/saved/' . $record->id)
            ->assertStatus(200)
            ->assertJson(["message" =>"Search record successfully removed."]);
    }
}
