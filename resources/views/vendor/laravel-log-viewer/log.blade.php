...
<div class="container-fluid">
    <div class="row">
        <div class="col sidebar mb-3">
            <h1><i class="fa fa-calendar" aria-hidden="true"></i> MyWork SalesForce Error Log Viewer</h1>
            ...
            @else
                <table id="table-log" class="table table-striped" data-ordering-index="{{ $standardFormat ? 2 : 0 }}">
                    ...
                    @foreach($logs as $key => $log)
                        @if($log['level_class'] && $log['level_class'] == 'info')
                            <tr data-display="stack{{{$key}}}">
                                ...
                                <td class="text">
                                    ...
                                    @endif
                                        {!! substr($log['text'], 0, strpos($log['text'], 'User Info:')) !!}
                                    @if (isset($log['in_file']))
                                        <br/>{{{$log['in_file']}}}
                                    @endif
                                    @if (strlen($log['text']) > 200)
                                        <div class="stack" id="stack{{{$key}}}"
                                             style="display: none; white-space: pre-wrap;">{!! '</span>' . substr($log['text'], strpos($log['text'], 'User Info:')) !!}
                                        </div>
                                    @endif
                                </td>
                             </tr>
